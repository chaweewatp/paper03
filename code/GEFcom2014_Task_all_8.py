...
model = 2
list_input=[[0,14,3]]  #[task(0-14), hour(0-23), method(2-5)]
...
for item in list_input:
    ...
    price = {
        'Task_1': [34.02, 28.26, 24.82, 22.9, 21.59, 21.33, 21.55, 22.49, 26.25, 28.95, 32.17, 32.44, 34.25, 35.97, 38.05,
                   37.77, 40.87, 37.51, 35.44, 35.28, 37.55, 38.41, 35, 32.31],
        'Task_2': [33.8, 34.24, 30.17, 27.4, 27.22, 29.29, 33.37, 35.48, 42.09, 45.25, 48.05, 49.32, 50.51, 51.7, 54.01,
                   58.36, 58.97, 53.08, 49.54, 47.87, 47.1, 45.27, 45.07, 40],
        'Task_3': [38.21, 33.66, 30.81, 29.29, 30.2, 32.49, 37.17, 48.54, 52.05, 52.99, 56.7, 61, 70.58, 76.75, 94.74,
                   100.77, 113.37, 96.91, 74.8, 65.9, 63.91, 62.8, 54, 48.65],
        'Task_4': [39.29, 31.85, 26.91, 24.9, 24.8, 24.63, 27.94, 30.35, 32.97, 38.86, 45.96, 50.29, 52, 59.61, 63.19,
                   73.77, 77.67, 70.25, 57.26, 52.86, 51.65, 50.73, 45.55, 40.11],
        'Task_5': [49.72, 42.62, 39.32, 38.53, 39.02, 38.54, 39.11, 47.08, 50.81, 56.7, 61.64, 67.63, 74.11, 80.36, 85.93,
                   108.56, 109, 81.4, 72.1, 62.14, 60.91, 59.4, 55.99, 52.9],
        'Task_6': [38.94, 32.69, 28.62, 29, 22.6, 22.14, 22.9, 32.11, 34.79, 36.14, 42.3, 49.17, 50.06, 51.08, 51.19, 52.01,
                   53.43, 52.96, 50.13, 47.86, 47.73, 47.19, 43.87, 42.96],
        'Task_7': [52.6, 55.79, 42.94, 41.96, 40.74, 41.54, 43.1, 50.63, 58.59, 65.72, 77.44, 93.76, 109.34, 125.56, 138,
                   164.02, 175.02, 155, 130.17, 100.53, 86.06, 81.91, 67.74, 58.61],
        'Task_8': [63.45, 59.55, 57.26, 50.85, 48.73, 51.53, 54.57, 68.72, 76.62, 92.69, 135.3, 155.99, 172.77, 226.22,
                   262.19, 292.06, 293.49, 264.13, 202.6, 154.59, 132.14, 123.12, 88.12, 71.33],
        'Task_9': [64.38, 63.02, 61.28, 55.38, 50.31, 50.09, 57.11, 62.43, 73.29, 84.72, 130, 159.77, 192.58, 265, 291.01,
                   313.7, 318.47, 288.37, 178.28, 148.64, 107.45, 90.65, 83.81, 72.81],
        'Task_10': [61.49, 58.79, 56.62, 53.76, 47.6, 43.56, 44.1, 48.14, 60.55, 66.24, 77.81, 84.9, 103.51, 110.2, 128.12,
                    128.33, 119.25, 107.92, 85.53, 78.76, 76.8, 77.11, 74.38, 63.15],
        'Task_11': [46.4, 39.41, 34.94, 30.69, 30.62, 32.81, 36.04, 41.56, 50.23, 52.74, 57.4, 57.94, 59.31, 60.91, 62.42,
                    63.01, 62.98, 62.62, 58.58, 56.62, 54.34, 50.93, 49.42, 45.28],
        'Task_12': [40.3, 37.93, 31.1, 30.86, 25.87, 28.63, 35.45, 38.96, 42.86, 47.69, 54.39, 54.14, 57.55, 59.24, 61.06,
                    71.06, 65.69, 59.51, 54.17, 51.18, 50.49, 49.01, 46.08, 43.22],
        'Task_13': [42.33, 38.3, 38.45, 36.62, 35.5, 38.15, 40, 41.55, 47.64, 49.67, 50.55, 52.7, 50.3, 48.39, 45.85, 42.79,
                    55.09, 77.82, 70.24, 61.78, 57.26, 52.29, 50.48, 45.6],
        'Task_14': [46.5, 42.59, 41.21, 40.48, 40.72, 40.7, 40.29, 39.79, 43.32, 46.34, 46.78, 46.87, 44.69, 44.33, 44.64,
                    43.94, 55.06, 80.55, 72.64, 61.48, 55.15, 51.07, 44.3, 40.93],
        'Task_15': [66.15, 71.85, 69.78, 69.68, 64.56, 68.87, 96.34, 113.22, 108.14, 97.22, 102, 112.69, 90.41, 88.66,
                    85.22, 100, 124.37, 161.95, 126.25, 113.92, 107.26, 89.02, 85.4, 86.13]}
    mu = df1['Zonal Price'].mean()
    sd = df1['Zonal Price'].std()
    # print(mu + 2 * sd)
    # print(mu + 8 * sd)
    df1['price type'] = [0 if temp_price < mu + 2 * sd else 1 if temp_price < mu + 8 * sd else 2 for temp_price in
    ...
    if model==2:
        ...
        if item[2]==0:
            ...
            if forecast_df.iloc[hour, 8] == 0:  # 'normal price'
                ...
                if len(df3.index) == 0:
                    df3 = df1.drop(list(set(np.where(df1['price type'] != 2)[0]))).copy()
                    df3 = df3.reset_index(drop=True)
            ...
            input1, target1, target2 = get_train_data(df3, df3['Forecasted Total Load'].max(),
                                                      df3['Forecasted Zonal Load'].max(), df3['Zonal Price'].max(),q1,q2)
            net = nl.net.newff([[0, 1], [0, 1]], [5, 2])
            err = net.train(input1, target1, epochs=500, show=10, goal=0.01)

            input2 = get_test_data(np.asarray(forecast_df.loc[hour]), df3['Forecasted Total Load'].max(), df3['Forecasted Zonal Load'].max())
            ...
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_005/lower_bound'.format(task,model), '{}'.format(hour),list_lower_01[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_005/upper_bound'.format(task,model), '{}'.format(hour),list_upper_01[0])
        elif item[2]==1:
            ...
            list_upper_01.append(upper[0])
            list_lower_01.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_01/lower_bound'.format(task,model), '{}'.format(hour),list_lower_01[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_01/upper_bound'.format(task,model), '{}'.format(hour),list_upper_01[0])
        elif item[2]==2:
            ...
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_005/lower_bound'.format(task,model), '{}'.format(hour),list_lower_02[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_005/upper_bound'.format(task,model), '{}'.format(hour),list_upper_02[0])
        elif item[2]==3:
            ...
            list_upper_02.append(upper[0])
            list_lower_02.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_01/lower_bound'.format(task,model), '{}'.format(hour),list_lower_02[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_01/upper_bound'.format(task,model), '{}'.format(hour),list_upper_02[0])

        elif item[2]==4:  #4 for QR_0.25
            a=0.1
            q1=0.25
            q2=0.75
            list_upper_02=[]
            ...
            list_upper_02.append(upper[0])
            list_lower_02.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_025/lower_bound'.format(task,model), '{}'.format(hour),list_lower_02[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_025/upper_bound'.format(task,model), '{}'.format(hour),list_upper_02[0])
        ...
        if item[2] == 0:
            ...
            if forecast_df.iloc[hour, 8] == 0:  # 'normal price'
                ...
                df3 = df3.reset_index(drop=True)
                if len(df3.index) == 0:
                    df3 = df1.drop(list(set(np.where(df1['Hour'] != hour)[
                                                0]))).copy()  # drop column which day_type not equal to 0 or select only day_type =0
                    df3 = df3.reset_index(drop=True)
                    # df3 = df3.drop(list(set(np.where(df3['day_type'] != day_type)[0])))
                    # df3 = df3.reset_index(drop=True)
                    # df3 = df3.drop(list(set(np.where(df3['holiday'] != is_holiday)[0])))
                    # df3 = df3.reset_index(drop=True)
                    df3 = df3.drop(list(set(np.where(df3['price type'] != 0)[0]))).copy()
                    df3 = df3.reset_index(drop=True)
            elif forecast_df.iloc[hour, 8] == 1:  # 'high price'
                df3 = df2.drop(list(set(np.where(df2['price type'] != 1)[0]))).copy()
                df3 = df3.reset_index(drop=True)
            ...
            input1, target1, target2 = get_train_data(df3, df3['Forecasted Total Load'].max(),
                                                      df3['Forecasted Zonal Load'].max(), df3['Zonal Price'].max(), q1,
                                                      q2)
            net = nl.net.newff([[0, 1], [0, 1]], [5, 2])
            err = net.train(input1, target1, epochs=500, show=10, goal=0.01)
            ...
            list_upper_01.append(upper[0])
            list_lower_01.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_005/lower_bound'.format(task, model),
                         '{}'.format(hour), list_lower_01[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_005/upper_bound'.format(task, model),
                         '{}'.format(hour), list_upper_01[0])
        elif item[2] == 1:
            ...
            list_upper_01.append(upper[0])
            list_lower_01.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_01/lower_bound'.format(task, model),
                         '{}'.format(hour), list_lower_01[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/mean_std_01/upper_bound'.format(task, model),
                         '{}'.format(hour), list_upper_01[0])
        elif item[2] == 2:
            ...
            list_upper_02.append(upper[0])
            list_lower_02.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_005/lower_bound'.format(task, model), '{}'.format(hour),
                         list_lower_02[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_005/upper_bound'.format(task, model), '{}'.format(hour),
                         list_upper_02[0])
        elif item[2] == 3:
            ...
            list_upper_02.append(upper[0])
            list_lower_02.append(lower[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_01/lower_bound'.format(task, model), '{}'.format(hour),
                         list_lower_02[0])
            firebase.put('GEFcom2014_spike/{}/results/model-{}/QR_01/upper_bound'.format(task, model), '{}'.format(hour),
                         list_upper_02[0])
    elif model==4:
        hour = item[1]
