...
def get_test_data(inp_df, max_total_load, max_zonal_load):
    output = np.asarray([[inp_df.iloc[ind,0]/max_total_load, inp_df.iloc[ind,1]/max_zonal_load] for ind in list(inp_df.index)])
    return output
...
# input1, target1, target2 = get_train_data(df1_1, df1_1['Forecasted Total Load'].max(), df1_1['Forecasted Zonal Load'].max(), df1_1['Zonal Price'].max())
# net = nl.net.newff([[0, 1], [0, 1]], [5, 2])
...
# forecast_df= forecast_df.reset_index(drop=True)
#
# input2 = get_test_data(forecast_df, df1_1['Forecasted Total Load'].max(), df1_1['Forecasted Zonal Load'].max())
# var_mean, var_std = get_sim_data(net, input2, df1_1['Zonal Price'].max())
...
input2 = get_test_data(forecast_df, df1_1['Forecasted Total Load'].max(), df1_1['Forecasted Zonal Load'].max())
lower_bound, upper_bound = get_sim_data(net, input2, df1_1['Zonal Price'].max())
